...
select YXBMZD_NM from YXBMZD where (YXBMZD_NM in (
SELECT AO_DATA FROM GSPUSER , GSPAURESULT WHERE OWNERID=GSPUSER.ID   
		AND AO_ID='YXSJBMQX'   
		AND CODE=@USERID 
		AND assid1='01999' ) 
		OR  EXISTS(SELECT 1 FROM  GSPUSER , GSPAURESULT WHERE OWNERID=GSPUSER.ID   
		AND AO_ID='YXSJBMQX'   
		AND CODE=@USERID 
		AND assid1='01999'
		AND AO_DATA='*'))
...
SELECT @CON=COUNT(*) FROM #NY
    ...
    SELECT  YXERPFH_GSID F_GSID,YXDDYWLX_NM F_QDLX,YXERPFH_YXBSC F_BMNM,YXERPFH_KH F_KHNM,ISNULL(YXKHXY_ZQ,'') F_ZQ,YXERPFH_NY F_NY,ISNULL(YXERPFH_JE,0) F_FHJE,0 F_HKJE FROM       YXERPFH LEFT JOIN YXKHXY ON YXKHXY_KHNM = YXERPFH_KH
	LEFT JOIN YXDDYWLX ON YXDDYWLX_BH=YXERPFH_C9
	WHERE YXERPFH_NY BETWEEN @KSNY AND @JSNY
    AND (YXERPFH_GSID=@GSID OR @GSID='') 
	AND (YXDDYWLX_NM=@QDLX OR @QDLX='')
	AND (YXERPFH_YXBSC=@BMNM OR @BMNM='')
	AND (YXERPFH_KH=@KHNM OR @KHNM='')
	 UNION ALL
	 SELECT ISNULL(YXRKD_GSID,''),ISNULL(YXRKD_QDLX,''),ISNULL(YXRKD_BMBH,''),ISNULL(YXRKD_KH,''),ISNULL(YXKHXY_ZQ,0) YXKHXY_ZQ,YXRKD_NY F_NY,0 F_FHJE,ISNULL(YXRKD_JE,0) F_HKJE
	 FROM YXRKD  LEFT JOIN YXKHXY ON YXKHXY_KHNM = YXRKD_KH
	 WHERE  YXRKD_NY BETWEEN @KSNY AND @JSNY
     AND (YXRKD_GSID=@GSID OR @GSID='') 
	 AND (YXRKD_QDLX=@QDLX OR @QDLX='')
	 AND (YXRKD_BMBH=@BMNM OR @BMNM='')
	 AND (YXRKD_KH=@KHNM OR @KHNM='')
...
END
	...
	AND (YXERPFH_KH=@KHNM OR @KHNM='')
	 ...
	 UPDATE #TEMP SET F_RJQK= CASE WHEN  F_TS=0 THEN 0 ELSE F_FHJE/F_TS END, F_RJHK=CASE WHEN  F_TS=0 THEN 0 ELSE F_HKJE/F_TS END
   	 --更新日均回款(日均欠款/日均回款)
	 UPDATE #TEMP SET F_SJHKZQ=CASE WHEN F_RJHK=0 THEN 0 ELSE F_RJQK*100/F_RJHK END
...
BEGIN
	 ...
	 SELECT  YXKHZD_KHMC F_KH,LSBZDW_DWMC F_HSDW,YXRYZD_RYXM F_JBR ,YXDDYWLX_MC F_QDLX ,YXBMZD_BMMC F_BMNM ,F_KSRQ ,F_JSRQ ,F_TS ,F_RJQK ,F_RJHK ,F_SJHKZQ  FROM #TEMP
	 LEFT JOIN YXKHZD ON YXKHZD_NM=F_KHNM
	 LEFT JOIN LSBZDW ON LSBZDW_DWBH=F_GSID
